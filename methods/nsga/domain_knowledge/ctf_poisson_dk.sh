#!/bin/bash
#SBATCH --job-name=ctf_poisson_dk
#SBATCH --output=logs/ctf_poisson_dk/output_%A_%a.txt
#SBATCH --error=logs/ctf_poisson_dk/error_%A_%a.txt
...
OUTPUT_FILE="data/ctf/nsga_poisson_dk/output_nsga_percent_${percent}_seed_${seed}_alpha_${alpha}.dot"
...
/usr/bin/time -v causal-testing discover \
    ...
    -o "$OUTPUT_FILE" \
    -a $alpha \
    -e "partials/ctf_poisson_percent_${percent}_seed_${seed}_excluded.dot" \
    -i "partials/ctf_poisson_percent_${percent}_seed_${seed}_included.dot" \
