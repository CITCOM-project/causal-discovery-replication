#!/bin/bash
#SBATCH --job-name=ctf_vaccine_dk
#SBATCH --output=logs/ctf_vaccine_dk/output_%A_%a.txt
#SBATCH --error=logs/ctf_vaccine_dk/error_%A_%a.txt
...
OUTPUT_FILE="data/ctf/nsga_vaccine_dk/output_nsga_percent_${percent}_seed_${seed}_alpha_${alpha}.dot"
...
/usr/bin/time -v causal-testing discover \
    ...
    -o "$OUTPUT_FILE" \
    -a $alpha \
    -e "partials/ctf_vaccine_percent_${percent}_seed_${seed}_excluded.dot" \
    -i "partials/ctf_vaccine_percent_${percent}_seed_${seed}_included.dot" \
