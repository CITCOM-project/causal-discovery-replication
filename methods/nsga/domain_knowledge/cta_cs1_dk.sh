#!/bin/bash
#SBATCH --job-name=cta_cs1_dk
#SBATCH --output=logs/cta_cs1_dk/output_%A_%a.txt
#SBATCH --error=logs/cta_cs1_dk/error_%A_%a.txt
...
OUTPUT_FILE="data/cta/nsga_dk_cs1/output_nsga_percent_${percent}_seed_${seed}_alpha_${alpha}.dot"
...
/usr/bin/time -v causal-testing discover \
    -t NSGADiscovery \
    -d data/cta/cs1_data.csv \
    -o "$OUTPUT_FILE" \
    -a $alpha \
    -e "partials/cta_cs1_percent_${percent}_seed_${seed}_excluded.dot" \
    -i "partials/cta_cs1_percent_${percent}_seed_${seed}_included.dot" \
    --technique-kwargs random_seed="$seed" population_size=7 num_parents_mating=4 \
    -V width height intensity num_lines_abs num_shapes_abs num_lines_unit num_shapes_unit
