#!/bin/bash
#SBATCH --job-name=sachs_sd
#SBATCH --output=logs/sachs_sd/output_%A_%a.txt
#SBATCH --error=logs/sachs_sd/error_%A_%a.txt
...
OUTPUT_FILE="data/sachs/nsga_sd/output_nsga_alpha_${val}_seed_${seed}.dot"
...
/usr/bin/time -v causal-testing discover \
    -t NSGADiscovery \
    -d data/sachs/b2camp.csv data/sachs/cd3cd28_aktinhib.csv data/sachs/cd3cd28_g0076.csv data/sachs/cd3cd28_icam2.csv \
