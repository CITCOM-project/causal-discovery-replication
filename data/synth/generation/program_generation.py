...
def generate_if_else_body(effect, causes, conditional_causes):
    ...
    additional_else_body_nodes = random.sample(sorted(nodes_not_in_else_body), random.randint(0, len(nodes_not_in_else_body)))
...
def write_statement_stack_to_python_file(
    ...
    with open(os.path.join(target_directory_path, f"{program_name}.py"), "w") as program_file:
    ...
    for node in nodes:
        for edge in edges:
            dag = generate_dag(
                ...
                0, 
                seed=63, 
                dot_path=f"data/synth/data/dag_node_{node}_edge_{int(edge*100)}.dot"
            )
            generate_program(
                dag,
                0,
                target_directory_path="data/synth/data/",
                program_name=f"program_node_{node}_edge_{int(edge*100)}"
            )